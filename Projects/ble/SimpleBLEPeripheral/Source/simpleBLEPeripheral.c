...
static void readDs18b20Data1( uint8* mData, uint8 flagrom)
{
    ...
    {
        flag = 1;                 //ÎÂ¶ÈÎª¸º
        a = (tem_l>>4);           //È¡ÎÂ¶ÈµÍ4Î»Ô­Âë
        b = (tem_h<<4)& 0xf0;     //È¡ÎÂ¶È¸ß4Î»Ô­Âë
        /*²¹Âë-Ô­Âë×ª»»
          ¸ºÊýµÄ²¹ÂëÊÇÔÚÆäÔ­ÂëµÄ»ù´¡ÉÏ, ·ûºÅÎ»²»±ä, ÆäÓà¸÷Î»È¡·´, ×îºó+1
        */
        tem_h = ~(a|b) + 1;       //È¡ÕûÊý²¿·ÖÊýÖµ£¬²»·ûºÅÎ»

        tem_l = ~(a&0x0f) + 1;    //È¡Ð¡Êý²¿·ÖÔ­Öµ£¬²»º¬·ûºÅÎ»
    }
    else
    {
        flag = 0;                 //ÎªÕý
        a = tem_h<<4;
        a += (tem_l&0xf0)>>4;     //µÃµ½ÕûÊý²¿·ÖÖµ
        b = tem_l&0x0f;           //µÃ³öÐ¡Êý²¿·ÖÖµ
        tem_h = a;                //ÕûÊý²¿·Ö
        tem_l = b&0xff;           //Ð¡Êý²¿·Ö
    }

    uint8 sensor_data_value[2];
    unsigned char FRACTION_INDEX[16] = {0, 1, 1, 2, 2, 3, 4, 4, 5, 6, 6, 7, 7, 8, 9, 9 };//Ð¡ÊýÖµ²éÑ¯±í
    sensor_data_value[0] = FRACTION_INDEX[tem_l]; //²é±íµÃÐ¡ÊýÖµ
    sensor_data_value[1] = tem_h| (flag<<7);      //ÕûÊý²¿·Ö£¬°üÀ¨·ûºÅÎ»

    float ft = sensor_data_value[1] + ((float)sensor_data_value[0])/10.0;
    mData[2] = sensor_data_value[1];
    mData[3] = sensor_data_value[0];

    DS18B20_Init();               //¸´Î»18B20
    DS18B20_Write(0xCC, 1);      //·¢³öÌø¹ýROMÆ¥Åä²Ù×÷
