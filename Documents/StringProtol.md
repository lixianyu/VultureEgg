...
<package body> ::= <蓝牙UUID>;<unix时间戳>;<加速度数据>;<陀螺仪数据>;<温度数据>;<湿度数据> | <unix时间戳>;<温度数据>;<湿度数据>;<光照数据>
  ...
  <加速度数据> ::= <x-axis-a>,<y-axis-a>,<z-axis-a>
            <x-axis-a>,<y-axis-a>,<z-axis-a> ::= <float format,string sending,单位g>
  <陀螺仪数据> ::= <x-axis-z>,<y-axis-z>,<z-axis-z>
            <x-axis-z>,<y-axis-z>,<z-axis-z> ::= <float format,string sending,单位自定>
    <温度数据> ::= <temp01>,<temp02>,...,<temp14>
	<temp01>,<temp02>,...,<temp14> ::= <float format,string sending,单位℃>
    <湿度数据> ::= <float format,string sending,单位%RH>
    <光照数据> ::= <float format,string sending,单位lux>
```
